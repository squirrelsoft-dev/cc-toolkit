...
# Syncs templates (commands, agents, rules, hooks, skills) from cc-toolkit into an existing project.
...
# Sync a nested template directory (skills/<name>/*) into the project
# Usage: sync_skills_dir <template_subdir> <target_subdir>
sync_skills_dir() {
  local src="$SCRIPT_DIR/templates/$1"
  local dst="$CLAUDE_DIR/$2"
  ...
  for skill_src in "$src"/*/; do
    ...
    local skill_dst="$dst/$skill_name"
    mkdir -p "$skill_dst"

    # Add/update files within the skill
    for file in "$skill_src"*; do
      [ -f "$file" ] || continue
      local name
      name=$(basename "$file")
      local target="$skill_dst/$name"

      if [ ! -f "$target" ]; then
        cp "$file" "$target"
        success "Added $2/$skill_name/$name"
        added=$((added + 1))
      elif ! diff -q "$file" "$target" &>/dev/null; then
        cp "$file" "$target"
        success "Updated $2/$skill_name/$name"
        updated=$((updated + 1))
      else
        skipped=$((skipped + 1))
      fi
    ...
    # Remove files within the skill that no longer exist in templates
    for file in "$skill_dst"/*; do
      [ -f "$file" ] || continue
      local name
      name=$(basename "$file")
      if [ ! -f "$skill_src/$name" ]; then
        rm "$file"
        warn "Removed $2/$skill_name/$name (no longer in templates)"
        removed=$((removed + 1))
      fi
    done
  done

  # Remove skill directories that no longer exist in templates
  for skill_dst_dir in "$dst"/*/; do
    [ -d "$skill_dst_dir" ] || continue
    local skill_name
    skill_name=$(basename "$skill_dst_dir")
    if [ ! -d "$src/$skill_name" ]; then
      rm -rf "$skill_dst_dir"
      warn "Removed $2/$skill_name/ (no longer in templates)"
      removed=$((removed + 1))
    fi
  done
}
